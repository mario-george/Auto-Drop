                      ...
                      >
                        <div className="flex w-full justify-between items-center px-2 pt-2 ">
                          <div className="flex items-center gap-x-2">
                            <p className={`  ${value == index.toString() ? `text-white`:`text-[#263238]` } text-white whitespace-nowrap`}>{nameOfShippingComp} </p>
                            <p className={`text-sm  ${value == index.toString() ? `text-[#CCE0DB]`:`text-[#263238]` }  whitespace-nowrap `}>
                              {" "}

                              {option.shipping_method || option.service_name}
                            </p>
                            <span className={`text-xs${value == index.toString() ? `text-white`:`text-[#263238]` } tab:text-lg whitespace-nowrap`}>{durationToDeliver}</span>
                            <span className={` ${value == index.toString() ? `text-[#CCE0DB]`:`text-[#263238]` }`}>
                              {option.estimated_delivery_time}
                            </span>
                            {/*    <p className="text-xs px-2">
                        </p> */}
                            <p className={`text-xs tab:text-lg   tab:pr-2 ${value == index.toString() ? `text-[#F4B1AC]`:`text-[#C1121F]` }`}>
                              {CurrencyFormatter(
