...
export default function useOrderRenderer({orderData,translationMessages}: { orderData: any;translationMessages: {[key:string]:string};}){
    let OrderDataComponent = <> </>
    let {orderDetails} = translationMessages
if(orderData == "fail"){
    OrderDataComponent = <>Order not found</>
}else if (orderData.length == 0){
    OrderDataComponent =  <div className="flex space-s-3 items-center text-green-500">
    <Spinner />
