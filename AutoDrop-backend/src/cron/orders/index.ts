...
let paginateOptions =  {
  ...
  populate: [
    {
      path: "merchant",
      select: "aliExpressToken sallaToken",
      populate: {
        path: "aliExpressToken",
    ...
    do{
    const result = await Order.paginate({ tracking_order_id: { $ne: null }},paginateOptions);
orders = result.docs
if (orders.length) {
  orders.forEach(async (order:any) => {
    ...
    if (orderData.tracking_order_id) {
      ...
      let aliexpressToken : any = order.merchant.aliExpressToken;
      ...
      const { data: trackingResponse } = await MakeRequest(body,tokenInfo);
      const orderStatus =
